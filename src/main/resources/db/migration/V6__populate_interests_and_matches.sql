...
-- Eliminamos la restricción antigua por si acaso existe con el nombre viejo
ALTER TABLE matches DROP CHECK chk_different_children;

-- Renombramos las columnas a los nombres que espera tu Entidad Java y el DESCRIBE
ALTER TABLE matches
    CHANGE COLUMN child_a_id child_request_id BIGINT NOT NULL,
    CHANGE COLUMN child_b_id child_target_id BIGINT NOT NULL;

-- Re-creamos la restricción con los nuevos nombres
ALTER TABLE matches
